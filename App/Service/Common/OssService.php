...
class OssService
{
    ...
    const FILE_TYPE_USER_AVATAR = 'userAvatar';

    const FILE_TYPE_RESOURCES_URL = 'resourcesUrl';

    const FILE_TYPE_GOODS_IMG = 'goodsImg';
    const FILE_TYPE_CATEGORY_IMG = 'categoryImg';
    const FILE_TYPE_PROP_IMG = 'propImg';
    const FILE_TYPE_BANNER_IMG = 'bannerImg';
    const FILE_TYPE_FEEDBACK_IMG = 'feedbackImg';
    const FILE_TYPE_WEDDING_PHOTOS_URL = 'weddingPhotosUrl';
    const FILE_TYPE_PROP_ANIMATION_URL = 'animationUrl';
    const FILE_TYPE_PRO_TRANSPARENT_URL = 'transparentUrl';
    const FILE_TYPE_MUSIC_URL = 'musicUrl';
    const FILE_TYPE_MUSIC_JPG = 'musicJpg';
    const FILE_TYPE_INVITATION_URL = 'invitationUrl';
    const FILE_TYPE_PICTURE_URL = 'pictureUrl';
    const FILE_TYPE_COVER_URL = 'coverUrl';
    const FILE_TYPE_INVITATION_COVER_URL = 'invitationCoverUrl';

    const FILE_TYPE_COVER = 'cover'; //封面
    const FILE_TYPE_URL = 'url'; //时刻资源
...
// 6.验证签名
    ...
    static function jpgColor($src)
    {
        $poster = $src . '?x-oss-process=image/average-hue';
        $config = Config::getInstance()->getConf('ALI_OSS.HOST');
        $httpClient = new HttpClient($config . "/" . $poster);
        $response = $httpClient->get();
        $data = json_decode($response->getBody(), 1);
        return $data['RGB'] ?? '';
    }

    static function jpgSize($cover)
    {
        if (empty($cover)) {
            return null;
        }
        $cover = OssService::splicingOssHost($cover);
        $img_info = getimagesize($cover);
        return $img_info;
    }
